...
--freeze_local
```
The `--freeze_local` flag will freeze the local generators for during training, which preserve the spatial disentanglement. However, for datasets that has a large geometric difference from the real faces (e.g. BitMoji), you may want to remove this argument. In fact, we found that our model is still able to preserve the disentanglement within a few thousand steps of fine-tuning all modules.

Note that the dataloader for adaptation is compatiable with rosinality's implementation, so you can use the same LMDB datasets for fine-tuning SemanticStyleGAN. Given that most datasets for style adaptation has only very limited number of images, the model could overfit quickly. You may want to look at the visualization samples for early stopping.
