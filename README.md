...
|[Toonify](https://github.com/seasonSH/SemanticStyleGAN/releases/download/1.0.0/Toonify-512x512.pt)  | Fine-tuned on the [Toonify](https://github.com/justinpinkney/toonify) dataset.
...
<img src="https://semanticstylegan.github.io/images/traverse.gif" width="450"/>  
...
```
Here, `/results/samples/000000_latent.npy` is the latent code either generated by `visualize/generate.py` or output by `visualize/invert.py`. You can also ignore the `--latent` argument for 
...
In `visualize/generate_components.py`, we provide a script for visualizing the components synthesized by SemanticStyleGAN, where we gradually add more local generators
